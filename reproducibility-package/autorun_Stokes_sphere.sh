...
# Fig - 10 Convergence: p = 16, 1st-kind, solver_tol = 1e-5:
printf "StokesBEM on a sphere: Convergence\n" >> $OUT
for i in {3..7}
	do eval $DIR$kernel -p 16 -fixed_p -recursions $i -ncrit 400 | grep -i "error on a sphere" >> $OUT
done

eval $DIR$kernel -p 16 -fixed_p -recursions 8 | grep -i "error on a sphere" >> $OUT

printf ">>>StokesBEM on a sphere: Convergence test completed!\n"

# Fig - 11 Residual History : p = 16, recursions = 6, 1st-kind:
printf "StokesBEM on a sphere: Residual History\n" >> $OUT
printf "fixed-p residual history\n" >> $OUT
eval $DIR$kernel -recursions 6 -p 16 -fixed_p -ncrit 400 | grep -i fmm_req_p >> $OUT
printf "relaxed-p residual history\n" >> $OUT
eval $DIR$kernel -recursions 6 -p 16 -ncrit 150 | grep -i fmm_req_p >> $OUT
